...
Brick.brick_geometry = new THREE.BoxGeometry(1.9, 1.8, 5.4, 1, 1, 1);
Brick.brick_material = new THREE.MeshStandardMaterial({ color: 0xcb4154 });
class Log {
    constructor() {
...
Log.offest = 3.0;
Log.thickness = 0.8;
Log.geometry = new THREE.BoxGeometry(Log.thickness, Log.thickness, 9.0, 1, 1, 1);
Log.material = new THREE.MeshStandardMaterial({ color: 0x211911, emissiveMap: loader.load('/8k_sun.png'), emissive: 0xe25822 });
function create_logLayer(height) {
...
function create_logStack(stack_height) {
...
function create_brickCircle(radius) {
    let all = new THREE.Group();
...
function create_lights(radius_in) {
...
export class Campfire {
    constructor(stack_height_in, radius_in) {
